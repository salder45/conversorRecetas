...
<html>
  <head>
    ...
    <title><g:message code="materiaPrima.buscar" /></title>
        ...
        <tbody>
          <tr class="prop">
            ...
            <td valign="top" class="name">
              <label for="nombre"><g:message code="materiaPrima.ingrediente" default="Ingrediente" /></label>
            </td>
            <td valign="top" class="value ${hasErrors(bean: materiaPrima , field: 'nombre', 'errors')}">
        <g:textField name="nombre" value="${materia?.nombre}" />      
        </tr>
        </tbody>
      </table>
      <div class="buttons">
        <span class="button"><g:submitButton name="editar" class="list" value="${message(code: 'materiaPrima.buscar.materia', default: 'Buscar')}" /></span>
      </div>
    </g:form>
  </div>
<g:javascript>
